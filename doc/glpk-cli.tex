...
%
%  Copyright (C) 2015 Heinrich Schuchardt,
%  <xypron.glpk@gmx.de>
%
...
\newcommand{\glpkCliVersion}{1.0.2}
\newcommand{\glpkVersionMajor}{4}
\newcommand{\glpkVersionMinor}{56}
...
It is hosted at
\linebreak\href{http://glpk-cli.sourceforge.net/}{http://glpk-java.sourceforge.net/}.
...
\lstset{language=bash,escapeinside={~}{.}}
\begin{lstlisting}
csc /r:lib_glpk_cli.dll test.cs
\end{lstlisting}
...
\linebreak\href{ftp://gnu.ftp.org/gnu/glpk/}{ftp://gnu.ftp.org/gnu/glpk/}.
...
are in directory w32 for 32 bit Windows and in w64 for 64 bit Windows.
The name of the created library is
glpk\_\glpkVersionMajor\_\glpkVersionMinor.dll for revision
\glpkVersionMajor.\glpkVersionMinor.

A precompiled version of GLPK is provided at
\href{http://winglpk.sourceforge.net}{http://winglpk.sourceforge.net}.

The library has to be in the search path for binaries. Either copy the library
to a directory that is already in the path (e.g.
C:\textbackslash windows\textbackslash system32) or update the path in the
system settings of Windows.

\section{GLPK for C\#/CLI }
\index{library}
\subsection{Source}
The source code to compile the GLPK for C\#/CLI library is provided at
\linebreak\href{http://glpk-cli.sourceforge.net}{http://glpk-cli.sourceforge.net}.

\subsection{Linux}
\index{Linux}
The GLPK for C\#/CLI library can be compiled from source code. Follow the
instructions in file INSTALL provided in the source distribution.

The usual installation path for the library is
/usr/local/lib/glpk-cli/libglpk-cli.so.
\subsection{Windows}
\index{Windows}
The GLPK for C\#/CLI library can be compiled from source code. The build and make
files are in directory w32 for 32 bit Windows and in w64 for 64 bit Windows.
The name of the created library is libglpk\_cli\_native.dll.

A precompiled version of GLPK for C\#/CLI is provided at
...
\section{GLPK for C\#/CLI allembly}
The source code to compile the GLPK for C\#/CLI allembly is provided at
\linebreak\href{http://glpk-java.sourceforge.net}{http://glpk-java.sourceforge.net}.

\subsection{Linux}
\index{Linux}
The GLPK for C\#/CLI assembly can be compiled from source code. Follow the
instructions in file INSTALL provided in the source distribution.

The usual installation path for the assembly is /usr/local/lib/glpk-cli/libglpk-cli.dll.
...
\subsection{Windows}
\index{Windows}
The GLPK for C\#/CLI assembly can be compiled from source code. The build and make
files are in directory w32 for 32 bit Windows and in w64 for 64 bit Windows.
The name of the created library is libglpk\_cli.dll.

A precompiled version of GLPK including GLPK for C\#/CLI is provided at
\linebreak\href{http://winglpk.sourceforge.net}{http://winglpk.sourceforge.net}.
...
GLPK.glp\_cli\_arc\_get\_data,\linebreak
GLPK.glp\_cli\_vertex\_data\_get, and
GLPK.glp\_cli\_vertex\_get\_data can be used.
